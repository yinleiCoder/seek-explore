...
export default function PostPage({
  ...
  return (
    <main className="grid p-2 md:p-0 grid-cols-12 container mx-auto gap-y-2 xl:px-2">
      ...
      <div className="hidden w-full lg:block lg:col-start-10 lg:col-end-13 lg:row-start-1 relative lg:row-span-5">
        <details
          className="text-zinc-900 rounded-lg p-2 sticky top-4 max-h-[80vh] overflow-hidden overflow-y-auto bg-white dark:bg-zinc-800 dark:text-zinc-300 duration-200 border shadow-lg select-none"
          open
        >
          ...
          <ul className="mt-4 text-base font-medium">
            {toc.map(heading => {
              return (
                <li
                  key={heading.text}
                  className="py-1 hover:bg-zinc-100 hover:text-primary rounded-md"
                >
                  <a
                    href={`#${heading.text}`}
